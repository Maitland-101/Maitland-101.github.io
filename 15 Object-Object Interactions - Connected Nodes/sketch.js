...
let reach = 150; //how far connections between points may be
...
function draw() {
  background(220);
  if(mouseIsPressed){
    nodes.push(new csNode(mouseX,mouseY));//draw as long as mouse is down;
  }
  for(let n of nodes){
    //looping by itemsince we don't plan on deleting any objects from the array
    n.move();
    n.connect(nodes);
    n.display();
  }
...
class csNode{
  ...
  constructor(x,y){ //called once per object created
    //properties related to postiin/rendering
    this.x = x;
    this.y = y;
    this.size = 20;
  ...
  connect(nodeArray){
    //check if the current point is close to any other points and if so join with a line
    stroke(this.c);
    for(let n of nodes){
      //this.x this.y  n.x n.y
      if(n !== this){ // make sure not to compare to self
        let d = dist(this.x, this.y, n.x, n.x);
